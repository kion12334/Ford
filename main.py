...
print("🚀 Starting Discord Bot on Railway with Supabase...")
print("=" * 50)
...
def load_quarantine():
    """Load quarantine data from JSON"""
    try:
...
def load_last_salary():
    """Load last salary time from JSON"""
    try:
...
def load_last_business_profit():
    """Load last business profit time from JSON"""
    try:
...
def save_last_business_profit(last_profit_time):
    """Save last business profit time to JSON"""
...
async def on_ready():
    ...
    # Connect to database
    if await db.connect():
        # Load data from database, overwriting JSON data
...
async def on_message(message):
    ...
    if user_id in bot.afk_users and not message.content.startswith('!'):
        ...
        try:
            ...
            save_data()
            asyncio.create_task(async_save_warnings())  # not warnings, but data includes warnings? Actually afk is separate. We'll leave save_data for now.
        except:
            pass
...
async def next_country_round(guild):
    ...
    game["winners"] = []
    game["round_count"] += 1   # increment round counter
    ...
    if game["game_type"] == "flag":
        # Use flag emoji for now; you can replace with image URL if you have country codes
        ...
        # If you have country codes for images, uncomment:
        # if 'code' in country:
        #     flag_url = f"https://flagcdn.com/256x192/{country['code']}.png"
...
async def afk(ctx, *, reason="No reason"):
    ...
    save_data()
    # AFK not in DB yet, but we can add later if needed
    embed = create_embed(
        "⏸️ AFK Set",
...
async def mute(ctx, member: discord.Member, minutes: int = 10, *, reason="No reason"):
    ...
    save_data()
    # Mutes not in DB yet, but we can add later
...
async def warn(ctx, member: discord.Member, *, reason="No reason"):
    ...
    # warnings dict structure: {user_id: count} (but we need per-guild)
    # Actually your bot.warnings might be per-guild. Let's assume it's per-guild as in your original code.
    # I'll adapt to per-guild:
...
def main():
    ...
    print("=" * 50)
    print("🤖 DISCORD BOT STARTING ON RAILWAY WITH SUPABASE")
    print("=" * 50)
